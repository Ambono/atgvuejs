<template>
    <!-- Extras -->
    <ul class="order-summary__extras order-summary--amend">
        <transition-group v-on:before-enter="prepareAnimation" v-on:enter="animateIn" v-on:leave="animateOut" mode="out-in">
            
            <li class="order-summary__extra" v-for="(extra) in extras" :key="'e' + extra.id" :v-if="extras">
                <!-- Icon -->
                ...
                <!-- /Icon -->
                <h4 class="order-summary__extra-title">{{ extra.title }}</h4>
                <h5 class="order-summary__extra-quantity">{{ $t('components.shared.orderSummary.modules.localExtras.order-summary__extra-quantity', { extraQuantity : extra.quantity, price : extra.prices[0].unit_price.toFixed(2) }) }}</h5>
                <!-- Remove -->
                <transition v-on:before-enter="prepareAnimation" v-on:enter="animateInDelay" v-on:leave="animateOut">
                <div class="order-summary__remove">
...
export default {
    props: ["amend"],
    computed: {
        // Computed property to retrive any extras added to the basket.
        extras() { 
