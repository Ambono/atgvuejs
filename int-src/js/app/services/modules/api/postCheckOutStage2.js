...
import { postCheckOutStage2ApiPath } from "./helpers/apiPaths";
import { PhoneNumber } from "../../../../core/objects/PhoneNumber";
import * as Sentry from '@sentry/browser';
import  i18n  from '../../../../i18n-setup';

export const postCheckOutStage2 = (apiEventBus) => {
       
    var errormessage={
        errmsgpart1:i18n.t('service.modules.api.postCheckoutStage2.errmsgpart1'),
        errmsgpart2:i18n.t('service.modules.api.postCheckoutStage2.errmsgpart2') 
       };  
  
    apiEventBus.$on("postCheckoutCardPaymentStage2", (callback) => {
        ...
        const bookingReferenceNumber = store.getters.getBookingRef;

        console.log("Payment stage 2, booking reference number is : " + bookingReferenceNumber);
        ...
        requestData.booking_reference_number = bookingReferenceNumber;

        const request = axios.post(postCheckOutStage2ApiPath(appSettings.core.client), requestData);
        ...
        request.then((response) => {
            ...
            if(response.data.errorCode !== 0) {
                ...
                const errmsg = this.errormessage.errmsgpart1 + appSettings.core.phoneNumber + this.errormessage.errmsgpart2 + response.data.booking_references_no + " " + response.data.errorText;
        ...
        request.catch((error) => {
            ...
            console.log(error);            
            const errmsg = this.errormessage.errmsgpart1 + appSettings.core.phoneNumber + this.errormessage.errmsgpart2 + bookingReferenceNumber + " " + error;
            Sentry.captureException(new Error(errmsg));
            callback({ status: false, errorText: errmsg });
        });
    });
}